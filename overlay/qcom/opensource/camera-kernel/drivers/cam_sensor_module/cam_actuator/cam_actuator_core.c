...
#define MI_PARKLENS CAM_ACTUATOR
...
static int32_t parklens_thread_func(void *arg)
{
	...
	struct parklens_wake_lock parklens_wakelock = {0};
	...
	soc_private =
		(struct cam_actuator_soc_private *)a_ctrl->soc_info.soc_private;
	i2c_set = &(a_ctrl->i2c_data.parklens_settings);
	power_info = &soc_private->power_info;
	...
	if (rc < 0) {
		...
		CAM_ERR(MI_PARKLENS,
			"acquire wakelock at parklens thread failed");
	} else {
		parklens_wake_lock_acquire(&parklens_wakelock);
		CAM_DBG(MI_PARKLENS,
			"acquire wakelock at parklens thread");
	}
	...
	void *arg)
{
	...
	case CAM_ACTUATOR_PACKET_OPCODE_INIT:
		...
		for (i = 0; i < csl_packet->num_cmd_buf; i++) {
			...
			case CAMERA_SENSOR_CMD_TYPE_PWR_DOWN:
				...
				if (PARKLENS_INVALID != parklens_atomic_read(
	...
	case CAM_ACTUATOR_PACKET_OPCODE_PARKLENS: {
		CAM_INFO(MI_PARKLENS, "Received parklens buffer");
		...
		if (rc < 0) {
			CAM_ERR(CAM_ACTUATOR,
				"Failed:parse parklens settings: %d", rc);
			delete_request(i2c_reg_settings);
			goto end;
		}

		rc = parklens_thread_trigger(a_ctrl);
		if (rc < 0) {
			CAM_ERR(CAM_ACTUATOR,
				"Failed to trigger parklens thread: %d", rc);
			goto end;
		}
		break;
	}
