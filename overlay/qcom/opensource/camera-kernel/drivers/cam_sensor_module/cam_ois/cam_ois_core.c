...
static int cam_ois_fw_download(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	#endif
	/* Fuxi's longest OIS firmware basename plus ".coeff" needs 33 bytes. */
	char                               name_prog[64] = {0};
	...
	snprintf(name_coeff, sizeof(name_coeff), "%s.coeff", o_ctrl->ois_name);

	snprintf(name_prog, sizeof(name_prog), "%s.prog", o_ctrl->ois_name);

#if defined(CONFIG_TARGET_PRODUCT_FUXI) || defined(CONFIG_TARGET_PRODUCT_NUWA)
	snprintf(name_mem, sizeof(name_mem), "%s.mem", o_ctrl->ois_name);
	#endif
...
#if defined(CONFIG_TARGET_PRODUCT_FUXI)
		/*
		 * Fuxi's BU24721 uses a vendor-specific flash protocol and gyro
		 * calibration sequence.  The generic byte-burst downloader writes
		 * the same firmware with the wrong transaction width/order and leaves
		 * the userspace OIS formatter without samples (dataCount == 0).
		 */
		if (o_ctrl->opcode.customized_ois_flag) {
			CAM_DBG(CAM_OIS,
				"Using BU24721 native init path, flag: %u",
				o_ctrl->opcode.customized_ois_flag);
			rc = bu24721_ois_pkt_download(o_ctrl);
			if (rc) {
				CAM_ERR(CAM_OIS,
					"BU24721 native init failed: %d", rc);
				goto pwr_dwn;
			}
			goto ois_init_done;
		}
...
int cam_ois_driver_cmd(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	...
	case CAM_STOP_DEV:
		...
		break;
	case CAM_FLUSH_REQ:
		/* Xiaomi camera issues this during teardown; OIS has no queue to flush. */
